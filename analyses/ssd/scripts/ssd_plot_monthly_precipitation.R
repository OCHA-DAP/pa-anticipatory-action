...
floodscan_dir <- paste0(data_private_dir, "/processed/ssd/floodscan")
...
df_fs_month <- df_fs %>% 
  ...
  summarise(max_cell_month = max(max_cell),mean_cell_month = max(mean_cell),percentile_2_month=max(percentile_2)) %>% 
  mutate(date = lubridate::as_date(paste0(year, month, '01'), format='%Y%m%d'))
...
# plt_fs
# 
# plt_fs <- ggplot(df_fs_month, aes(x=date, y=mean_cell_month,color=rainy_season))+#, group=ADM1_EN)) +
...
plt_fs <- ggplot(df_fs_month, aes(x=date, y=percentile_2_month,color=rainy_season))+#, group=ADM1_EN)) +
  scale_color_manual(values=c(yes="#C25048", no="#cccccc"))+#(values = c(apple = "red", avocado = "green", ...)
  facet_wrap(~ADM1_EN)+
  geom_line(aes(group = 1))+
  labs(y='mean fraction of cell flooded', x='Date')+
