...
df_fadm = pd.read_csv(
    f"../Data/FewsNetWorldPop/somalia_fewsnet_worldpop_admin1.csv", index_col=0
)
adm1c = "ADMIN1"
...
df_ntl = (
    df_fadm.drop(["year", "month"], axis=1)
    .groupby(by="date", axis=0, as_index=False)
    .sum()
)
df_ntl.head(10)
...
def get_national_abs_criterion(row, period, phase, threshold):
    """Return 1 if percentage of population in row for period in phase "phase"
    or higher, equals or larger than.
...
def get_national_increase_criterion(row, period, phase, threshold):
    """Return 1 if for row percentage in >="phase" projected at Period minus
    percentage currently (CS) in >="phase" is expected to be larger than
    Threshold.
...
df_ntl["natl_criterion_ML1_3_20"] = df_ntl.apply(
    lambda x: get_national_abs_criterion(x, "ML1", 3, 20), axis=1
)
df_ntl["natl_criterion_ML1_3_5in"] = df_ntl.apply(
    lambda x: get_national_increase_criterion(x, "ML1", 3, 5), axis=1
)
df_ntl["natl_criterion_ML1_4_2half"] = df_ntl.apply(
    lambda x: get_national_abs_criterion(x, "ML1", 4, 2.5), axis=1
)

df_ntl["natl_criterion_ML2_3_20"] = df_ntl.apply(
    lambda x: get_national_abs_criterion(x, "ML2", 3, 20), axis=1
)
df_ntl["natl_criterion_ML2_3_5in"] = df_ntl.apply(
    lambda x: get_national_increase_criterion(x, "ML2", 3, 5), axis=1
)
df_ntl["natl_criterion_ML2_4_2half"] = df_ntl.apply(
    lambda x: get_national_abs_criterion(x, "ML2", 4, 2.5), axis=1
)


# determine whether national trigger is met

df_ntl["national_trigger_ML1"] = np.where(
    (df_ntl["natl_criterion_ML1_3_20"] == 1)
    & (
        (df_ntl["natl_criterion_ML1_3_5in"])
        == 1 | (df_ntl["natl_criterion_ML1_4_2half"] == 1)
    ),
    1,
    0,
)
df_ntl["national_trigger_ML2"] = np.where(
    (df_ntl["natl_criterion_ML2_3_20"] == 1)
    & (
        (df_ntl["natl_criterion_ML2_3_5in"])
        == 1 | (df_ntl["natl_criterion_ML2_4_2half"] == 1)
    ),
    1,
    0,
)
...
display(
    national_activations_ML1.round(2)
    .groupby(["year", "month"], as_index=False)[
        "period", "perc_CS_3p", "perc_CS_4", "perc_ML1_3p", "perc_ML1_4p"
    ]
    .agg(lambda x: list(x))
)
display(
    national_activations_ML2.round(2)
    .groupby(["year", "month"], as_index=False)[
        "period", "perc_CS_3p", "perc_CS_4", "perc_ML2_3p", "perc_ML2_4p"
    ]
    .agg(lambda x: list(x))
)
...
dict_natl_activ = {}
dict_natl_activ["past_ML1"] = {
    "df": national_activations_ML1,
...
dict_natl_activ["past_ML2"] = {
    "df": national_activations_ML2,
...
def plot_natl_trig(
    dict_natl_trigger,
    adm0c="admin0Name",
    shape_path="../Data/Shapefiles/som_adm_undp_shp/Som_Admbnda_Adm0_UNDP.shp",
):
    ...
    for d in range(2009, 2021):
        ...
        regions = dict_natl_trigger["adm0c"].loc[
            dict_natl_trigger["year"] == d
        ]
        if len(regions) > 0:
...
def plot_aff_dates(
    ...
    gdf = gpd.read_file(shape_path)
    df_geo = gdf[["admin0Name", "geometry"]].merge(
        df_d, left_on="admin0Name", right_on="adm0c", how="left"
    )
...
def get_trigger_increase(row, period, phase, threshold):
    """Return 1 if for row percentage in >="phase" at period minus percentage
    in >="phase" currently (CS) is expected to be larger than threshold.
...
df_fadm["trigger_ML1_3_20"] = df_fadm.apply(
    lambda x: get_trigger(x, "ML1", 3, 20), axis=1
)
df_fadm["trigger_ML1_3_5ir"] = df_fadm.apply(
    lambda x: get_trigger_increase(x, "ML1", 3, 5), axis=1
)
df_fadm["trigger_ML1_4_2half"] = df_fadm.apply(
    lambda x: get_trigger(x, "ML1", 4, 2.5), axis=1
)

df_fadm["trigger_ML2_3_20"] = df_fadm.apply(
    lambda x: get_trigger(x, "ML2", 3, 20), axis=1
)
df_fadm["trigger_ML2_3_5ir"] = df_fadm.apply(
    lambda x: get_trigger_increase(x, "ML2", 3, 5), axis=1
)
df_fadm["trigger_ML2_4_2half"] = df_fadm.apply(
    lambda x: get_trigger(x, "ML2", 4, 2.5), axis=1
)


# analyse endorsed trigger applied at subnational level
subnatl_activations_ML1 = df_fadm.loc[
    (df_fadm["trigger_ML1_3_20"] == 1)
    & (
        (df_fadm["trigger_ML1_3_5ir"] == 1)
        | (df_fadm["trigger_ML1_4_2half"] == 1)
    )
]
subnatl_activations_ML2 = df_fadm.loc[
    (df_fadm["trigger_ML2_3_20"] == 1)
    & (
        (df_fadm["trigger_ML2_3_5ir"] == 1)
        | (df_fadm["trigger_ML2_4_2half"] == 1)
    )
]

display(
    subnatl_activations_ML1.groupby(["year", "month"], as_index=False)[
        admc, "perc_CS_3p", "perc_ML1_3p", "perc_ML1_4p"
    ].agg(lambda x: list(x))
)
display(
    subnatl_activations_ML2.groupby(["year", "month"], as_index=False)[
        admc, "perc_CS_3p", "perc_ML2_3p", "perc_ML2_4p"
    ].agg(lambda x: list(x))
)
...
)  # remove max column width for ADMIN1 to print fully
subnatl_activations_ML1.groupby(["year", "month"], as_index=False)[admc].agg(
    lambda x: list(x)
)
...
display(
    subnatl_activations_ML1.groupby(["ADMIN1"], as_index=False)[["date"]].agg(
        lambda x: list(x)
    )
)
display(
...
dict_subnatl_activ["subnatl_ML1"] = {
    "df": subnatl_activations_ML1,
    "trig_cols": ["CS_3p", "ML1_3p", "ML1_4p"],
    "desc": "At least 20% of ADMIN1 population in IPC3+ at ML1 AND (increase by 5 percentage points in ADMIN1 pop. projected in IPC3+ compared to current state OR at least 2.5% of ADMIN1 population projected at IPC4+ by ML1)",
}

dict_subnatl_activ["subnatl_ML2"] = {
    "df": subnatl_activations_ML2,
    "trig_cols": ["CS_3p", "ML1_3p", "ML1_4p"],
    "desc": "At least 20% of ADMIN1 population in IPC3+ at ML2 AND (increase by 5 percentage points in ADMIN1 pop. projected in IPC3+ compared to current state OR at least 2.5% of ADMIN1 population projected at IPC4+ by ML2)",
}
...
def plot_regions_trig(
    df_trig,
    adm0c="admin0Name",
    adm1c="admin1Name",
    shape_path="../Data/Shapefiles/som_adm_undp_shp/Som_Admbnda_Adm1_UNDP.shp",
):
    gdf = gpd.read_file(shape_path)
...
def plot_aff_dates(
    ...
    gdf = gpd.read_file(shape_path)
    df_geo = gdf[["admin1Name", "geometry"]].merge(
        df_d, left_on="admin1Name", right_on="ADMIN1", how="left"
    )
...
df_gadm = pd.read_csv(
    f"../Data/GlobalIPCProcessed/{country}_globalipc_admin1.csv", index_col=0
)
...
df_gadm["trigger_ML1_3_20"] = df_gadm.apply(
    lambda x: get_trigger(x, "ML1", 3, 20), axis=1
)
df_gadm["trigger_ML1_3_5ir"] = df_gadm.apply(
    lambda x: get_trigger_increase(x, "ML1", 3, 5), axis=1
)
df_gadm["trigger_ML1_4_2half"] = df_gadm.apply(
    lambda x: get_trigger(x, "ML1", 4, 2.5), axis=1
)

df_gadm["trigger_ML2_3_20"] = df_gadm.apply(
    lambda x: get_trigger(x, "ML2", 3, 20), axis=1
)
df_gadm["trigger_ML2_3_5ir"] = df_gadm.apply(
    lambda x: get_trigger_increase(x, "ML2", 3, 5), axis=1
)
df_gadm["trigger_ML2_4_2half"] = df_gadm.apply(
    lambda x: get_trigger(x, "ML2", 4, 2.5), axis=1
)


# analyse endorsed trigger applied at subnational level
glob_subnatl_activations_ML1 = df_gadm.loc[
    (df_gadm["trigger_ML1_3_20"] == 1)
    & (
        (df_gadm["trigger_ML1_3_5ir"] == 1)
        | (df_gadm["trigger_ML1_4_2half"] == 1)
    )
]
glob_subnatl_activations_ML2 = df_gadm.loc[
    (df_gadm["trigger_ML2_3_20"] == 1)
    & (
        (df_gadm["trigger_ML2_3_5ir"] == 1)
        | (df_gadm["trigger_ML2_4_2half"] == 1)
    )
]

print("ML1 below")
display(
    glob_subnatl_activations_ML1.groupby(["year", "month"], as_index=False)[
        admc, "perc_CS_3p", "perc_ML1_3p", "perc_ML1_4p"
    ].agg(lambda x: list(x))
)
print("ML2 below")
display(
    glob_subnatl_activations_ML2.groupby(["year", "month"], as_index=False)[
        admc, "perc_CS_3p", "perc_ML2_3p", "perc_ML2_4p"
    ].agg(lambda x: list(x))
)
...
df_gntl["natl_criterion_ML1_3_20"] = df_gntl.apply(
    lambda x: get_national_abs_criterion(x, "ML1", 3, 20), axis=1
)
df_gntl["natl_criterion_ML1_3_5in"] = df_gntl.apply(
    lambda x: get_national_increase_criterion(x, "ML1", 3, 5), axis=1
)
df_gntl["natl_criterion_ML1_4_2half"] = df_gntl.apply(
    lambda x: get_national_abs_criterion(x, "ML1", 4, 2.5), axis=1
)

df_gntl["natl_criterion_ML2_3_20"] = df_gntl.apply(
    lambda x: get_national_abs_criterion(x, "ML2", 3, 20), axis=1
)
df_gntl["natl_criterion_ML2_3_5in"] = df_gntl.apply(
    lambda x: get_national_increase_criterion(x, "ML2", 3, 5), axis=1
)
df_gntl["natl_criterion_ML2_4_2half"] = df_gntl.apply(
    lambda x: get_national_abs_criterion(x, "ML2", 4, 2.5), axis=1
)
...
df_gntl["national_trigger_ML1"] = np.where(
    (df_gntl["natl_criterion_ML1_3_20"] == 1)
    & (
        (df_gntl["natl_criterion_ML1_3_5in"] == 1)
        | (df_gntl["natl_criterion_ML1_4_2half"] == 1)
    ),
    1,
    0,
)
df_gntl["national_trigger_ML2"] = np.where(
    (df_gntl["natl_criterion_ML2_3_20"] == 1)
    & (
        (df_gntl["natl_criterion_ML2_3_5in"] == 1)
        | (df_gntl["natl_criterion_ML2_4_2half"] == 1)
    ),
    1,
    0,
)
...
df_gntl_noagg = pd.read_excel(
    f"../Data/GlobalIPC/{country.lower()}_globalipc_newcolumnnames.xlsx",
    index_col=0,
)
...
df_gntl_noagg.rename(
    columns={
        "reported_pop_CS": "pop_CS",
        "reported_pop_ML1": "pop_ML1",
        "reported_pop_ML2": "pop_ML2",
    },
    inplace=True,
)
...
df_gntl_noagg = df_gntl_noagg[
    (df_gntl_noagg["date"].notnull()) & (df_gntl_noagg[f"ADMIN0"].notnull())
]


# rows with country and then a colon, indicate the numbers on national level
df_gntl_noagg = df_gntl_noagg[
    df_gntl_noagg["ADMIN0"].str.contains(f"{country}:")
]
...
df_gntl_noagg["natl_criterion_ML1_3_20"] = df_gntl_noagg.apply(
    lambda x: get_national_abs_criterion(x, "ML1", 3, 20), axis=1
)
df_gntl_noagg["natl_criterion_ML1_3_5in"] = df_gntl_noagg.apply(
    lambda x: get_national_increase_criterion(x, "ML1", 3, 5), axis=1
)
df_gntl_noagg["natl_criterion_ML1_4_2half"] = df_gntl_noagg.apply(
    lambda x: get_national_abs_criterion(x, "ML1", 4, 2.5), axis=1
)

df_gntl_noagg["natl_criterion_ML2_3_20"] = df_gntl_noagg.apply(
    lambda x: get_national_abs_criterion(x, "ML2", 3, 20), axis=1
)
df_gntl_noagg["natl_criterion_ML2_3_5in"] = df_gntl_noagg.apply(
    lambda x: get_national_increase_criterion(x, "ML2", 3, 5), axis=1
)
df_gntl_noagg["natl_criterion_ML2_4_2half"] = df_gntl_noagg.apply(
    lambda x: get_national_abs_criterion(x, "ML2", 4, 2.5), axis=1
)


# determine whether national trigger is met
df_gntl_noagg["national_trigger_ML1"] = np.where(
    (df_gntl_noagg["natl_criterion_ML1_3_20"] == 1)
    & (
        (df_gntl_noagg["natl_criterion_ML1_3_5in"] == 1)
        | (df_gntl_noagg["natl_criterion_ML1_4_2half"] == 1)
    ),
    1,
    0,
)
df_gntl_noagg["national_trigger_ML2"] = np.where(
    (df_gntl_noagg["natl_criterion_ML2_3_20"] == 1)
    & (
        (df_gntl_noagg["natl_criterion_ML2_3_5in"] == 1)
        | (df_gntl_noagg["natl_criterion_ML2_4_2half"] == 1)
    ),
    1,
    0,
)
...
national_activations_ML1_noagg = df_gntl_noagg.loc[
    (df_gntl_noagg["national_trigger_ML1"] == 1)
]
national_activations_ML1_noagg["period"] = "ML1"
national_activations_ML1_noagg["adm0c"] = country

national_activations_ML2_noagg = df_gntl_noagg.loc[
    (df_gntl_noagg["national_trigger_ML2"] == 1)
]
national_activations_ML2_noagg["period"] = "ML2"
national_activations_ML2_noagg["adm0c"] = country

activation_frames_noagg = [
    national_activations_ML1_noagg,
    national_activations_ML2_noagg,
]
national_activations_noagg = pd.concat(activation_frames_noagg)

display(
    national_activations_ML1_noagg.round(2)
    .groupby(["year", "month"], as_index=False)[
        "period", "perc_CS_3p", "perc_CS_4", "perc_ML1_3p", "perc_ML1_4p"
    ]
    .agg(lambda x: list(x))
...
display(
    national_activations_ML2_noagg.round(2)
    .groupby(["year", "month"], as_index=False)[
        "period", "perc_CS_3p", "perc_CS_4", "perc_ML2_3p", "perc_ML2_4p"
    ]
    .agg(lambda x: list(x))
...
def get_national_relative_increase_criterion(row, period, phase, threshold):
    ...
    if row[["pop_CS", f"pop_{period}"]].isnull().values.any():
        ...
        round(
            (row[cols__ml].sum() - row[cols__cs].sum())
            / row[cols__cs].sum()
            * 100
        )
        >= threshold
...
# determine whether national trigger is met
df_gntl["natl_criterion_ML1_3_5relin"] = df_gntl.apply(
    lambda x: get_national_relative_increase_criterion(x, "ML1", 3, 5), axis=1
)
df_gntl["national_trigger_springanalysis"] = np.where(
    (df_gntl["natl_criterion_ML1_3_20"] == 1)
    & (
        (df_gntl["natl_criterion_ML1_3_5relin"] == 1)
        | (df_gntl["natl_criterion_ML1_4_2half"] == 1)
    ),
    1,
    0,
...
national_activations_ML1_springanalysis = df_gntl.loc[
    (df_gntl["national_trigger_springanalysis"] == 1)
]
...
display(
    national_activations_ML1_springanalysis.round(2)
    .groupby(["year", "month"], as_index=False)[
        "period", "perc_CS_3p", "perc_CS_4", "perc_ML1_3p", "perc_ML1_4p"
    ]
    .agg(lambda x: list(x))
...
# determine whether national trigger is met
df_gntl_noagg["natl_criterion_ML1_3_5relin"] = df_gntl_noagg.apply(
    lambda x: get_national_relative_increase_criterion(x, "ML1", 3, 5), axis=1
)
df_gntl_noagg["national_trigger_springanalysis"] = np.where(
    (df_gntl_noagg["natl_criterion_ML1_3_20"] == 1)
    & (
        (df_gntl_noagg["natl_criterion_ML1_3_5relin"] == 1)
        | (df_gntl_noagg["natl_criterion_ML1_4_2half"] == 1)
    ),
    1,
    0,
...
national_activations_ML1_springanalysis = df_gntl_noagg.loc[
    (df_gntl_noagg["national_trigger_springanalysis"] == 1)
]
national_activations_ML1_springanalysis["period"] = "ML1"
national_activations_ML1_springanalysis["adm0c"] = country
