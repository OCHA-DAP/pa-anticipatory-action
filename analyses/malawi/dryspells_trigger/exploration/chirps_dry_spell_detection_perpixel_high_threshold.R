...
st_bbox(mwi_adms) # spatial extent 
nrow(mwi_adms) # number of features (= spatial objects / adm2 regions) 
ncol(mwi_adms) # number of attributes
names(mwi_adms) # names of attributes
mwi_adms
plot(mwi_adms$geometry) # visual inspection

mwi_adms_spatial_extent <- st_bbox(mwi_adms)
...
year_by_adm2 <- crossing(year_list, mwi_adm2_ids$ADM2_PCODE) # create list with all year * adm2 combinations
names(year_by_adm2)[2] <- 'pcode'
year_by_adm2$year <- as.character(year_by_adm2$year)

# year_by_adm1 <- crossing(year_list, mwi_adm1_ids$ADM1_PCODE) # create list with all year * adm1 combinations
# names(year_by_adm1)[2] <- 'pcode'
# year_by_adm1$year <- as.character(year_by_adm1$year)
...
#                                 left_join(mwi_adm1_ids, by = 'ID') %>%
#                                 relocate(ADM1_PCODE, .after = ID) %>% # move the identification columns to the front of the dated values
#                                 relocate(ADM1_EN, .after = ADM1_PCODE)
...
# select dataset
data <- all_years_cell_values_adm2
#data <- all_years_cell_values_adm1
...
#write.csv(dry_spells_details, file = paste0(dry_spell_processed_path, "dry_spells_details_per_pixel_adm2_300mm.csv"), row.names = FALSE)
...
#write.csv(dry_spells_during_rainy_season_list, file = paste0(dry_spell_processed_path, "dry_spells_during_rainy_season_list_per_pixel_adm2_300mm.csv"), row.names = FALSE)
...
# save
#write.csv(df, file = paste0(dry_spell_processed_path, "ds_dates_per_pixel_adm2_300mm.csv"), row.names = FALSE)
...
#write.csv(complete_list,  file = paste0(dry_spell_processed_path, "complete_list_per_pixel_adm2_300mm.csv"), row.names = FALSE)
...
# save
#write.csv(adm2_ds_counts,  file = paste0(dry_spell_processed_path, "ds_counts_per_pixel_adm2_300mm.csv"), row.names = FALSE)
