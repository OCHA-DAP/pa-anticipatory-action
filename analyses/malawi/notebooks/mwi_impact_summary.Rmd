...
```{r cor-all, dpi=300, include=T, message=FALSE, warning=FALSE, fig.height=3, fig.align="center", fig.cap='Correlation matrix of variables across all regions'}

plt_cor_all
```

We only see weak correlations when comparing against our measure of dry spells. More dry spells occurring at a given time is weakly correlated with higher populations in a food security crisis in the following year. Contrary to what would be expected, our dry spells measure is negatively correlated (although very weakly) with ASI. 

These results suggest that perhaps our definition of dry spells is not capturing the kind of impact that we are interested in responding to.
