...
The results in the figure below are somewhat surprising as we see that the WRSI remains within the 'satisfactory' and 'sufficient' levels. Reports from FEWS NET clearly indicate problems with crop production as result of insufficient precipitation, which would mean that we'd expect to see WRSI values within the 'stress' category (10-60%) at some points. Perhaps averaging the results at the Admin 1 level causes us to miss important local variation, through which 'hotspots' of low WRSI would be evident at some points in time. 

```{r wrsi, dpi=300, include=T, message=FALSE, warning=FALSE, fig.height=5, fig.align="center", fig.cap='Mean WRSI for maize by region in Malawi from 2002 - 2015'}
...
```{r cor-cent, dpi=300, include=T, message=FALSE, warning=FALSE, fig.height=3, fig.align="center", fig.cap='Correlation matrix of variables in Central region'}

plt_cor_central
```

```{r cor-south, dpi=300, include=T, message=FALSE, warning=FALSE, fig.height=3, fig.align="center", fig.cap='Correlation matrix of variables in Southern region'}
