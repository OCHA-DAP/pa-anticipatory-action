## Skill of ECMWF's monthly forecast to predict dry spells
This notebook explores the skill of ECMWF's monthly forecasts to predict dry spells
To determine this skill, two parameters have to be set. Namely the cap of forecasted mm of precipitation during the month, and the probability of the precipitation being below this cap. This notebook explores several combinations of these two parameters. 
...
# for i, m in enumerate(df_pr_sep_lt.leadtime.unique()):
#     ax = fig.add_subplot(rows,colp_num,i+1)
#     df_pr_sep_lt[df_pr_sep_lt.leadtime==m].plot.bar(x="threshold",y="num_trig" ,figsize=(20, 12), color='#66B0EC',legend=False,ax=ax,label="trigger was incorrectly met")
#     df_pr_sep_lt[df_pr_sep_lt.leadtime==m].plot.bar(x="threshold",y="tp" ,figsize=(20, 12), color='#F2645A',legend=False,ax=ax,label="trigger was correctly met")
