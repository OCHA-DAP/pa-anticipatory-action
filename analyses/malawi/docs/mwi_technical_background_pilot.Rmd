...
- Defines a dry spell as at least 14 consecutive days with less than 2mm cumulative precipitation.
...
Moreover, we had to aggregate the dry spell data, which is at admin2, to admin1. We assigned an admin1 as experiencing a dry spell if at least **`r min_adm_ds`** admin2's experienced a dry spell during that time. 

Lastly, we had to convert the daily dry spell data to monthly dry spell data. We assigned a month as experiencing a dry spell, if at least **`r min_days_ds`** days of that month were part of a dry spell. 
...
#### Methodology
Due to the smaller leadtime, we assumed the spatial accuracy is high enough to analyze the skill of the forecast at admin2. The raster cell valus of the CHIRPS-GEFS forecast were aggregated to admin2 by taking the mean value across all cells within the admin2. 
...
![](`r paste0(plot_dir,'/chirps_gefs/plot_mwi_chirpsgefs_15days_density.png')`)
...
![](`r paste0(plot_dir,'/chirps_gefs/mean_2mm_th_2_chirpsgefs_dryspell_hm.png')`)
...
![](`r paste0(plot_dir,'/chirps_gefs/mean_2mm_th_25_chirpsgefs_dryspell_hm.png')`)
