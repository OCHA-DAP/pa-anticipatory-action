...
#data_values <- data_max_values
data_values <- data_mean_values
...
data_values_long <- convertToLongFormat(data_values)
data_values_long$year <- lubridate::year(data_values_long$date) 
data_values_long$month <- lubridate::month(data_values_long$date) 
data_values_long$day <- lubridate::day(data_values_long$date) 
data_values_long$season_approx <- ifelse(data_values_long$month >= 10, data_values_long$year, ifelse(data_values_long$month <= 7, data_values_long$year - 1, 'outside rainy season')) # labels the rainy season which overlaps between two calendar years. uses first year as label.
...
# label rainy days
data_values_long$rainy_day_bin <-  ifelse(data_values_long$total_prec >= 4, 1, 0) # rainy day defined as having received at least 4mm
data_values_long$rainy_day_bin_2mm <-  ifelse(data_values_long$total_prec >= 2, 1, 0) # rainy day defined as having received at least 2mm
...
data_values_long <- merge(data_values_long, rainy_seasons, by = c('ID', 'pcode', 'season_approx'), all.x = T)
data_values_long$during_rainy_season_bin <-  ifelse(data_values_long$date >= data_values_long$onset_date & data_values_long$date <= data_values_long$cessation_date, 1, 0)
data_values_long$nth_day_of_rainy_season <- ifelse(data_values_long$during_rainy_season_bin == 1, as.numeric(difftime(data_values_long$date, data_values_long$onset_date, units = "days") + 1), NA) # +1 so first day of rainy season is labelled "one"

# find rainy streaks within each rainy season, adm2
#rainy_streaks <- data_values_long %>%
...
data_values_long$rollsum_14d_less_than_2_bin <- ifelse(data_values_long$rollsum_14d <= 2, 1, 0) # NOTE: this does not label all days that have less than 2mm because those in the first 13 days don't get flagged

# identify beginning, end and duration of dry spells per adm2 region (total <= 2mm)
dry_spells_confirmation_dates <- data_values_long %>%
                                  group_by(pcode) %>%        
