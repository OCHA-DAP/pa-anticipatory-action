...
#append main repo folder to path and import modules
path_mod = f"{Path(os.path.dirname(os.path.realpath(__file__))).parents[2]}/"
sys.path.append(path_mod)
from indicators.drought.process_rainfallforecasts import get_iri_data, plot_raster_boundaries, compute_raster_statistics, plot_spatial_columns
...
def main(download, config=None):
    country="malawi"
    #load drought config parameters
    if config is None:
        config = Config()
    ...
    #assuming that want to plot data of last available date, else change the value of iri_lastdate
    ...
    prob_threshold=50
    iri_df=compute_raster_statistics(adm_path,iri_ds_latest_array,iri_transform,prob_threshold)
    #plot the statistics. currently creates ugly plots though!
    ...
    fig_stats_bins = plot_spatial_columns(iri_df, ['max_cell_touched', 'max_cell', 'avg_cell', 'avg_cell_touched'],
                                          predef_bins=bins)
    fig_stats_bins.savefig(os.path.join(output_dir, f'IRI_statistics_L4_F{iri_lastdate_formatted}_Cbelow_bins.png'),
                           format='png')
