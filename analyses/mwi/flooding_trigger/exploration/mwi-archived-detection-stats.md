** This is an old version of calculating the GloFAS detection stats. Preserving here to allow for reproducing plots in the summary slide deck if needed.
...
for code, station in stations_adm2.items():
    ...
    for rp in rp_list:
        ...
        for lt in LEADTIMES: 
            df_glofas_event = get_glofas_activations(ds_glofas_reforecast_summary[code]
                                                           .sel(leadtime=lt)
            ...
            for source in event_sources: 
                ...
                dict_performance = get_clean_stats_dict(df_glofas_event, df_event, days_before_buffer, days_after_buffer)
