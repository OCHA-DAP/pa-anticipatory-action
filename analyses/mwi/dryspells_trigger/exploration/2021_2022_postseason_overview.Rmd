...
The map below shows which week (7-day period) after 1 Nov the season begun. Beige-brown indicates a November onset date while blue-purple indicates Dec. The darker colours capture onset dates later in the month.

Most of the Southern region had an early November start to its season while the northern part of the region saw its season begin in Dec. By `r  format((as.Date("2021-11-01") + onset_max), "%d %b")`, rains had begun on the entire region.
...
plot(static_r, 
     ...
     plg = list(title = "Onset Week",
                legend = onset_labels[-length(onset_labels)]),
     col = onset_palette[1:(length(onset_labels) - 1)]) # to display all dates in a month one color
