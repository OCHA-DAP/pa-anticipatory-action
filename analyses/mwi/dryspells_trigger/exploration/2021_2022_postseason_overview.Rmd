...
- November and Dec dry spells are very rare and difficult to distinguish from dry days that precede the rainy season. The 2021-2022 season suggests that the AA gain would be minimal if the trigger monitored them (as currently defined.) 
- March dry spells were frequent and could more easily be monitored especially before 15 March. They may offer an opportunity to broaden the scope of the AA framework and trigger.
...
- The observations made on the basis of the 2021-2022 season should be validated in a historical analysis.
...
### Streaks of Dry Days
Streaks of dry days occurred throughout the region. 

The longest streak of consecutive dry days during the entire rainy season is represented below. The longest streaks lasted between `r minmax(static_r$max_streak)[1]` and `r minmax(static_r$max_streak)[2]` days.
...
The trigger mechanism built on work done by WFP on the definition of dry spells. The shocks were defined as a minimum of 14 consecutive days with no more than 2mm cumulative rainfall. Alternative definitions refer to shorter periods with little rainfall as dry spells (see Q3 below). Yet other definitions instead look for streaks of dry days, where each day is allowed up to a certain amount of rainfall (typically 2-4mm). 

During the 2021-2022, streaks of dry days were common. Every area in the region experienced a dry spell of at least 7 consecutive dry days and most areas experienced a streak of 10+ days. By contrast, fewer areas (`r round(100*length(cells(static_r[["n_14dplus_streaks"]])) / nrow(season_dates), 1)` % of the region) experienced streaks of 14+ dry days. Note that by the trigger definition no dry spells occurred in the region. Therefore, had 14-day dry days streaks been used in the trigger instead, more events would have been recorded and even more so had the streak been reduced in duration. Furthermore, 14-day streaks (may they be defined in dry days or with a maximum rainfall) are likely more localised than widespread, the latter being the type of shocks AA aims to target.

### Q3: How much rainfall was typical over 14-day periods during the rainy season? 

We now turn to the amount of rainfall allowed for a period to be considered a dry spell. The trigger definition set the maximum to 2mm over 14 days. The analysis revealed that this year very few (`r round(100*ecdf(in_season$roll_sum_next_14d)(2), 2)`%) periods received 2 or less mm. Even when allowing for 5 times more rainfall than the selected definition for dry spell (ie for 10mm per 14 days), the occurrence was still significantly less than 5% of the time, which would be a 1 in 20 year event if the trend held over the years. Note that the analysis looks at a single year and that thresholds should be set based on historical data over longer period of times. Note also that the analysis examines small areas of about 10 x 10 km and patterns can change when computing metrics for larger areas such as adm2 (district) or adm3 (traditional authority) regions (for instance, by averaging). 

Rainfall patterns in the 2021-2022 suggest that the dry spell definition used in the trigger captures very rare and localised events, and may be overly restrictive to get ahead of dry periods that could cause significant crop damage and humanitarian needs. Further work would be needed to evaluate alternative definitions to find one that may be better suited for AA. It would have to reflect events that are more widespread and frequent, and that are linked to observed crop damage and humanitarian needs. (Triggers would be adjusted to ensure that they would be met for shocks occurring once every 3 to 5 years.) Furthermore, using a less restrictive definition might result in improved predictability and thus, a better performing predictive trigger. This is speculative at the moment and could only be verified through extensive analysis.
