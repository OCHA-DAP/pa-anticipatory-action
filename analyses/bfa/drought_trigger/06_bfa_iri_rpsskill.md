...
This notebook explores the Ranked Probability Skill Score (RPSS) for IRI's seasonal forecast. In the end results were not really used. 
...
for l in leadtimes:
    ...
    remote_data = xr.open_dataset(
        f"http://iridl.ldeo.columbia.edu/home/.jingyuan/.NMME_seasonal_hindcast_verification/.monthly_RPSS_seasonal_hindcast_precip_ELR/.lead{l}/RPSS/dods",
        decode_times=False,
    )
...
comb_lt = xr.DataArray(
    data=data,
    dims=["leadtime", "T", "lat", "lon"],
    coords=dict(
        T=remote_data.T,
        lon=remote_data.lon,
        lat=remote_data.lat,
...
ds_clipped = (
    comb_lt.rio.set_spatial_dims(x_dim="lon", y_dim="lat")
    .rio.write_crs("EPSG:4326")
    .rio.clip(df_bound.geometry.apply(mapping), all_touched=True)
)
```
...
bins_clipped = np.linspace(
    ds_clipped["RPSS"].min(), ds_clipped["RPSS"].max(), 10
)
...
for l in [1, 3]:  # ds_clipped.leadtime.values:
    ...
    ds_sel = ds_clipped.sel(
        leadtime=l, T=ds_clipped.T.dt.month.isin(start_months_lt)
    )
    list_seasons = [
        (m + l) % 12 if (m + l) % 12 != 0 else 12
        for m in ds_sel["T"].dt.month.values
    ]
    title_list_clipped = [
        f"{seasons[s]} (issued in {m})"
        for s, m in zip(list_seasons, ds_sel["T"].dt.strftime("%b").values)
    ]
    g = ds_sel.RPSS.plot(
        col="T",
        #     row="leadtime",
...
for l in ds_clipped.leadtime.values:
    start_months_lt = [
        (m - l) % 12 if (m - l) % 12 != 0 else 12 for m in start_months
    ]
    ds_sel = ds_clipped.sel(
        leadtime=l, T=ds_clipped.T.dt.month.isin(start_months_lt)
    )
