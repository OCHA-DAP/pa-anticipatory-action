...
```python
da_season_below_rainy = da_season_below.sel(
    time=da_season_below.time.dt.month.isin(end_months_sel)
)
```

```python
da_plot = da_season_below_rainy.sel(
    time=da_season_below_rainy.time.dt.year.isin(range(2004, 2012))
).sortby("time")
da_plot_titles = [
...
for i, m in enumerate(end_months_sel):
    ...
    perc = np.percentile(
        da_season_reg_sel.values.flatten()[
            ~np.isnan(da_season_reg_sel.values.flatten())
        ],
        33,
    )
...
for i, m in enumerate(end_months_sel):
    ...
    perc = np.percentile(
        da_season_sel.values.flatten()[
            ~np.isnan(da_season_sel.values.flatten())
        ],
        33,
    )
    plt.axvline(perc, color="#C25048", label="lower tercile cap")
...
da_season_below_clip = da_season_below.rio.clip(
    gdf_reg["geometry"], all_touched=False
)
...
df_stats_reg["perc_bavg"] = (
    df_stats_reg[f"count_{adm0_pcode}"]
    / df_stats_reg_all[f"count_{adm0_pcode}"]
    * 100
)
...
df_stats_reg["rainy_seas"] = np.where(
    df_stats_reg.start_month.dt.month.isin(end_months_sel), 1, 0
)
df_stats_reg = df_stats_reg.sort_values("start_month")
df_stats_reg["rainy_seas_str"] = df_stats_reg["rainy_seas"].replace(
    {0: "outside rainy season", 1: "rainy season"}
)
df_stats_reg["year"] = df_stats_reg.end_month.dt.year
```

```python
df_stats_reg_rainy = df_stats_reg[
...
histo.facet(
    column=alt.Column(
        "season:N", sort=df_stats_reg_rainy.season.unique(), title="Season"
    ),
    data=df_stats_reg_rainy[["season", "perc_bavg"]],
    title=f"Distribution of percentage of area with below average precipitation from {df_stats_reg_rainy.end_month.dt.year.min()}-{df_stats_reg_rainy.end_month.dt.year.max()}",
)
```
...
df_rps_empirical_rainy = get_return_periods_dataframe(
    df_stats_reg_rainy, rp_var="perc_bavg", years=years, method="empirical"
)
```
...
fig, ax = plt.subplots()
ax.plot(
    df_rps_empirical_rainy.index,
    df_rps_empirical_rainy["rp"],
    label="empirical",
)
ax.legend()
...
df_rps_empirical_rainy["rp_round"] = 5 * np.round(
    df_rps_empirical_rainy["rp"] / 5
)
```
...
len(
    df_stats_reg_rainy[
        (df_stats_reg_rainy.start_month.dt.year >= 2017)
        & (
            df_stats_reg_rainy.perc_bavg
            >= df_rps_empirical_rainy.loc[3, "rp_round"]
        )
    ]
)
```
...
df_stats_reg["rainy_seas"] = np.where(
    df_stats_reg.end_month.dt.month.isin(end_months_sel), 1, 0
)
df_stats_reg = df_stats_reg.sort_values("start_month")
df_stats_reg["rainy_seas_str"] = df_stats_reg["rainy_seas"].replace(
    {0: "outside rainy season", 1: "rainy season"}
...
g = sns.catplot(
    ...
    col_wrap=4,
    kind="bar",
    palette={"outside rainy season": grey_med, "rainy season": hdx_blue},
    height=4,
    aspect=2,
)
g.map(
    plt.axhline,
    y=df_rps_empirical_rainy.loc[5, "rp_round"],
    linestyle="dashed",
    color=hdx_red,
    zorder=1,
...
g.map(
    plt.axhline,
    y=df_rps_empirical_rainy.loc[3, "rp_round"],
    linestyle="dashed",
    color=hdx_green,
    zorder=1,
