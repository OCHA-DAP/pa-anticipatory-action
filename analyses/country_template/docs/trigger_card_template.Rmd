...
kbl(snapshot_df,
  ...
  row_spec(c(1:4,10:16), extra_css = "line-height: 25px;") %>%
  row_spec(5:9, extra_css = "line-height: 45px;") %>%
  column_spec(1, width = "16em", extra_css = "vertical-align: bottom;") %>%
  column_spec(2:4, width = "22em") %>%
  pack_rows("Description", start_row = 1, end_row = 5, color = '#1bb580') %>%
  pack_rows("Performance", start_row = 6, end_row = 9, color = '#1bb580') %>%
  pack_rows("Shock", start_row = 10, end_row = 12, color = '#1bb580') %>%
  pack_rows("Activation", start_row = 13, end_row = 16, color = '#1bb580') %>%
kableExtra::footnote(general = "", symbol = c("Each trigger can be met independently from the others. Only 1 activation can occur per trigger in a given year."))
