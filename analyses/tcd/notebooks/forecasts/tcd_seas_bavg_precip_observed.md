...
# stats_reg_observed_path=os.path.join(chirps_exploration_dir,f"{iso3}_chirps_seasonal_bavg_stats_{''.join(adm_sel_str)}.csv")
...
da_season_below_clip=da_season_below.rio.clip(gdf_reg.geometry.apply(mapping),da_season_below.rio.crs,all_touched=False)
...
```{code-cell} ipython3
df_stats_reg_rainy=df_stats_reg[df_stats_reg.end_month.dt.month.isin([8,9])]
```
...
We assume years before 1993 were not reported in these sources. In that case we see a partial overlap with the reported years and observed below average precipitation. 

1993 and 2004 were specifically bad years in observed precipitation. 1996 as well, which might be the drought referred to by the year 1997 as impacts are sometimes felt later. 

With that same reasoning the 2001 drought might be caused by a deficit of rainfall in 2000 and 2009 drought by a deficit in 2007 and 2008. 2

011 also had a deficit in rainfall. 2017 barely saw any below average rainfall in our region of interest.
