...
```python
ds=xr.load_dataset("/Volumes/GoogleDrive/Shared drives/Predictive Analytics/CERF Anticipatory Action/General - All AA projects/Data/public/processed/tcd/chirps/monthly/tcd_chirps_monthly.nc")
```
...
```python
da_climate=ds_season_climate_quantile.precip
```
...
#     clim = gb.mean(dim="time")
    gb_bavg=gb.where(gb<da_climate)
    return gb_bavg.where(gb_bavg.isnull(),1)
#     return gb - clim
...
bla=da_season.map_blocks(calculate_anomaly, template=da_season).compute()
```

```python
bla_selm=bla.sel(time=bla.time.dt.month.isin(end_months_sel))
```

```python
da_plot=bla_selm.sel(time=bla_selm.time.dt.year.isin(range(2004,2006))).sortby("time")
...
```python
# ds_lt = da_season.apply(lambda x: x.where((x.isnull())|
