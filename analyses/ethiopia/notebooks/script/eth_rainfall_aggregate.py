...
country_data_processed_dir = os.path.join(
    config.DATA_DIR, config.PROCESSED_DIR, country
)
country_data_exploration_dir = os.path.join(
    config.DATA_DIR, "exploration", country
)
drought_data_exploration_dir = os.path.join(
    config.DATA_DIR, "exploration", "drought"
)
...
chirps_monthly_path = os.path.join(
    chirps_monthly_dir, "chirps_global_monthly.nc"
)
...
def load_iri(config):
    IRI_filepath = os.path.join(
        config.DROUGHTDATA_DIR, config.IRI_DIR, config.IRI_NC_FILENAME_CRS
    )
...
def interpolate_ds(ds, transform, upscale_factor):
    ...
    transform_interp = transform * transform.scale(
        len(ds.lon) / len(dsi.lon), len(ds.lat) / len(dsi.lat)
    )
...
ds_interp_clip = ds_interp.rio.set_spatial_dims(
    x_dim="lon", y_dim="lat"
...
ds_l2_interp = ds_interp.sel(
    F=cftime.Datetime360Day(2021, 1, 16, 0, 0, 0, 0), L=2
)


fig = plot_raster_boundaries_clip(
    [ds_l2, ds_l2_interp],
    adm1_bound_path,
    colp_num=2,
    predef_bins=np.arange(30, 70, 2.5),
    figsize=(32, 9),
    suptitle="Raw data of Jan 2021 forecast for MAM",
    title_list=["Original", "Interpolated"],
)
...
df_stats = compute_raster_statistics(
    adm1_bound_path,
    ds_l2_interp["prob_below"].values,
    transform_interp,
...
df_stats[["ADM1_EN"] + stats_cols + stats_perc_cols].style.highlight_max(
    color="orange", axis=0
)
...
plot_spatial_columns(
    df_stats,
    stats_cols,
    title="Analysis of IRI forecast",
    predef_bins=bins_list,
)
...
# compute the different aggregation methodologies per admin1
df_stats_adm2 = compute_raster_statistics(
    adm2_bound_path,
    ds_l2_interp["prob_below"].values,
    transform_interp,
    probability_threshold,
...
plot_spatial_columns(
    df_stats_adm2[df_stats_adm2.ADM1_EN == "Oromia"],
    stats_cols,
    title="Analysis of IRI forecast",
    predef_bins=bins_list,
)
...
def alldates_statistics(
    ds, transform, prob_threshold, perc_threshold, leadtime, adm_path
):
    ...
    for date in ds.F.values:
        ...
        df = compute_raster_statistics(
            adm_path, ds_date["prob_below"].values, transform, prob_threshold
        )
    ...
    for m in ["max_cell", "max_cell_touched", "avg_cell", "avg_cell_touched"]:
        df_hist[f"{m}_ge{prob_threshold}"] = np.where(
            df_hist[m] >= prob_threshold, 1, 0
        )
    df_hist[f"perc_threshold_ge{perc_threshold}"] = np.where(
        df_hist["perc_threshold"] >= perc_threshold, 1, 0
    )
    df_hist[f"perc_threshold_touched_ge{perc_threshold}"] = np.where(
        df_hist["perc_threshold_touched"] >= perc_threshold, 1, 0
    )

    df_hist["date_str"] = df_hist["date"].dt.strftime("%Y-%m")
    df_hist["pred_date"] = df_hist.date + pd.offsets.DateOffset(
        months=leadtime
    )
    # valid for period of 3 months, i.e. till 2 months later than first month
    df_hist["pred_date_end"] = df_hist.date + pd.offsets.DateOffset(
        months=leadtime + 2
    )
    df_hist["pred_date_form"] = df_hist["pred_date"].apply(
        lambda x: x.strftime("%b")
    )
    df_hist["pred_date_end_form"] = df_hist["pred_date_end"].apply(
        lambda x: x.strftime("%b %Y")
    )
    df_hist["forec_valid"] = df_hist[
        ["pred_date_form", "pred_date_end_form"]
    ].agg(" - ".join, axis=1)
...
df_hist[df_hist.date == "2021-01-16"][
    ["ADM1_EN"] + stats_cols + stats_perc_cols
]
...
prob_histograms(
    df_hist, stats_perc_cols, xlim=(0, df_hist.perc_threshold.max())
)


# binned values of max cell touched on adm1 for all history
pd.cut(
    df_hist.groupby("date").max()["max_cell_touched"], np.arange(30, 60, 2.5)
).value_counts().sort_index(ascending=False)


# binned values of avg cell on adm1 for all history
pd.cut(
    df_hist.groupby("date").max()["avg_cell"], np.arange(30, 60, 2.5)
).value_counts().sort_index(ascending=False)
...
df_hist_adm2 = alldates_statistics(
    ds_interp,
    transform_interp,
    probability_threshold,
    percentage_threshold,
...
prob_histograms(
    df_hist_adm2, stats_perc_cols, xlim=(0, df_hist.perc_threshold.max())
)


plot_spatial_columns(
    df_hist_adm2[df_hist_adm2.date == "2020-10-16"],
    stats_cols,
    title="Analysis of IRI forecast",
    predef_bins=bins_list,
)
...
df_hist[df_hist.perc_threshold > 0][
    ["date", "forec_valid", "ADM1_EN", "perc_threshold"]
].set_index(["date", "forec_valid", "ADM1_EN"])
...
fig = plot_raster_boundaries_clip(
    [
        ds_interp.sel(F=cftime.Datetime360Day(2021, 1, 16, 0, 0, 0, 0), L=2),
        ds_interp_40.sel(
            F=cftime.Datetime360Day(2021, 1, 16, 0, 0, 0, 0), L=2
        ),
    ],
    ...
    predef_bins=np.arange(30, 70, 2.5),
    figsize=(19, 6),
    title_list=["All cells", "Cells value >=40"],
    suptitle="Raw data of Jan 2021 forecast for MAM",
)
...
# compute aggregated values on adm1 for all dates since Jan 2017
df_hist_40 = alldates_statistics(
    ds_interp_40,
    transform_interp,
    probability_threshold,
    percentage_threshold,
...
act_adm1_40 = df_hist_40[df_hist_40.avg_cell >= 45][
    ["date", "forec_valid", "ADM1_EN", "avg_cell"]
]


act_adm1_40.sort_values(by=["date", "ADM1_EN"]).set_index(
...
df_hist_adm2[
    (df_hist_adm2.max_cell >= 47.5)
    & ~pd.MultiIndex.from_frame(df_hist_adm2[["date", "ADM1_EN"]]).isin(
        act_adm1_tuples
    )
][["date", "forec_valid", "ADM1_EN", "ADM2_EN", "max_cell"]].sort_values(
    by=["date", "ADM1_EN"]
).set_index(
...
df_hist[df_hist.forec_valid == "Mar - May 2021"][
    [
        "date",
        "forec_valid",
        "ADM1_EN",
        "perc_threshold",
        "perc_threshold_touched",
    ]
    + stats_cols
]
...
years_climate = slice(
    "1982-01-01", "2010-12-31"
)  # slice('2000-01-01', '2016-12-31')
...
for i in years:
    ...
    # for some unexplainable reason chirps of 1995 doesn't want to include the CRS when saving it to a file, so add write_crs here...
    ds_clip = (
        ds.rio.set_spatial_dims(x_dim=config.LONGITUDE, y_dim=config.LATITUDE)
...
for k, v in seasons.items():
    ...
    dict_ds_allseas[v]["year"] = (
        ds_months.groupby("time.year").sum(dim="time").rename({"year": "time"})
    )
    ...
    dict_ds_allseas[v]["anom"] = (
        dict_ds_allseas[v]["year"] - dict_ds_allseas[v]["avg"]
    )
...
for v in seasons.values():
    dict_ds_allseas[v]["anom"] = dict_ds_allseas[v]["anom"].expand_dims(
        {"seas": [v]}
    )
    # select data where precipitation is below average
    dict_ds_allseas[v]["belowavg"] = (
        dict_ds_allseas[v]["anom"]
        .where(dict_ds_allseas[v]["anom"].precip < 0)
        .dropna("time", how="all")
    )
#     dict_ds_allseas[v]["sel"]["seas_time"]=[f"{v} {y}" for y in dict_ds_allseas[v]["sel"]["time"].values]


ds_belowavg = xr.merge(
    [dict_ds_allseas[v]["belowavg"] for v in dict_ds_allseas.keys()]
)
...
np.unique(
    ds_belowavg.precip.values.flatten()[
        ~np.isnan(ds_belowavg.precip.values.flatten())
    ]
)
...
df_hist[df_hist.forec_valid == "Feb - Apr 2019"][
    [
        "date",
        "forec_valid",
        "ADM1_EN",
...
rpss_ds = xr.open_dataset(
    f"http://iridl.ldeo.columbia.edu/home/.jingyuan/.NMME_seasonal_hindcast_verification/.monthly_RPSS_seasonal_hindcast_precip_ELR/.lead{leadtime}/RPSS/dods",
    decode_times=False,
)
...
rpss_ds_clipped = (
    rpss_ds.rio.set_spatial_dims(x_dim="lon", y_dim="lat")
    .rio.write_crs("EPSG:4326")
    .rio.clip(df_bound.geometry.apply(mapping), df_bound.crs, all_touched=True)
)
...
rpss_bins_clipped = np.linspace(
    rpss_ds_clipped["RPSS"].min(), rpss_ds_clipped["RPSS"].max(), 10
)
seasons = {
    1: "JFM",
    2: "FMA",
...
list_seasons = [
    (m + leadtime) % 12 if (m + leadtime) % 12 != 0 else 12
    for m in rpss_ds_clipped["T"].dt.month.values
]
title_list_clipped = [
    f"{seasons[s]} (issued in {m})"
    for s, m in zip(
        list_seasons, rpss_ds_clipped["T"].dt.strftime("%b").values
    )
]
