...
# group to country
df_gnat = pd.read_csv(
    os.path.join(globalipc_dir, f"{country}_globalipc_admin0{suffix}.csv")
)
...
plot_ipclevels(
    df_gadm, adm1c=glob_adm1c, pop_col="pop_CS", figsize=(30, 45), width=10
)
...
def plot_aff_dates(
    ...
    """Plot a map with the values of col per admin1 for all dates included in
    df_d."""
...
df_fadm[[f"{c}_next" for c in shift_cols_ML1]] = df_fadm.groupby("ADM1_EN")[
    shift_cols_ML1
].shift(1)
df_fadm[[f"{c}_next2" for c in shift_cols_ML2]] = df_fadm.groupby("ADM1_EN")[
    shift_cols_ML2
].shift(2)
...
df_fadmt = pd.read_csv(
    f"Data/FewsNetProcessed/{country}_fewsnet_admin2{suffix}.csv", index_col=0
)
...
for a in df_fadmt.ADM1_EN.unique():
    for b in df_fadmt.ADM2_EN.unique():
        df_fadmt.loc[
            (df_fadmt.ADM1_EN == a) & (df_fadmt.ADM2_EN == b), f"ML1_next"
        ] = (
            df_fadmt.loc[
                (df_fadmt.ADM1_EN == a) & (df_fadmt.ADM2_EN == b), "ML1"
            ]
            .shift(1)
            .to_numpy()
        )
        df_fadmt.loc[
            (df_fadmt.ADM1_EN == a) & (df_fadmt.ADM2_EN == b), f"ML2_next2"
        ] = (
            df_fadmt.loc[
                (df_fadmt.ADM1_EN == a) & (df_fadmt.ADM2_EN == b), "ML2"
            ]
            .shift(2)
            .to_numpy()
        )
        df_fadmt.loc[
            (df_fadmt.ADM1_EN == a) & (df_fadmt.ADM2_EN == b), f"date_next"
        ] = (
            df_fadmt.loc[
                (df_fadmt.ADM1_EN == a) & (df_fadmt.ADM2_EN == b), "date"
            ]
            .shift(1)
            .to_numpy()
        )
        df_fadmt.loc[
            (df_fadmt.ADM1_EN == a) & (df_fadmt.ADM2_EN == b), f"date_next2"
        ] = (
            df_fadmt.loc[
                (df_fadmt.ADM1_EN == a) & (df_fadmt.ADM2_EN == b), "date"
            ]
