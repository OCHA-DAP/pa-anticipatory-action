...
### Remove the Annex if no trigger has more than 1 activation timepoint or adjust the Annex as needed if there are triggers with multiple activation timepoints
### Knit
```
...
perf_metrics_per_month_filename <- paste0(data_folder_path, country_pcode3, "_perf_metrics_table_per_month.csv") # only applies to triggers with multiple timepoints
learning_table_filename <- paste0(data_folder_path, country_pcode3, "_learning_table.csv")
ci <- 95 # which confidence intervals are presented? An integer between 0 and 100
...
df_a_var <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'var', trigger_id = 'a') # valid activation rate 
df_a_det <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'det', trigger_id = 'a') # detection rate
df_a_far <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'far', trigger_id = 'a') # false alarm rate
df_a_mis <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'mis', trigger_id = 'a') # miss rate
df_a_atv <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'atv', trigger_id = 'a') # probability of trigger activation

# compute width and position of CIs for trigger b
df_b_var <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'var', trigger_id = 'b') # valid activation rate 
df_b_det <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'det', trigger_id = 'b') # detection rate
df_b_far <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'far', trigger_id = 'b') # false alarm rate
df_b_mis <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'mis', trigger_id = 'b') # miss rate
df_b_atv <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'atv', trigger_id = 'b') # probability of trigger activation

# compute width and position of CIs for trigger c
df_c_var <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'var', trigger_id = 'c') # valid activation rate 
df_c_det <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'det', trigger_id = 'c') # detection rate
df_c_far <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'far', trigger_id = 'c') # false alarm rate
df_c_mis <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'mis', trigger_id = 'c') # miss rate
df_c_atv <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'atv', trigger_id = 'c') # probability of trigger activation

# compute width and position of CIs for the framework
df_fmwk_min <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'min', trigger_id = 'framework') # prob minimum framework activation 
df_fmwk_ful <- computeCIwidths(perf_metrics_df = perf_metrics_data, metric_name = 'ful', trigger_id = 'framework') # prob full framework activation 
...
<p style="color:black;font-size:16px;text-align:left;"> This table summarises the definition of the triggers and their performance in offering a reliable signal of the targeted severe shocks. The performance metrics are presented as ranges (called confidence intervals) to illustrate the inherent uncertainty of the measurements. The coloured graphs show the ranges within which the triggers will perform `r ci`% of the time. Green confidence intervals represent desired outcomes while red confidence intervals represent undesired outcomes or errors. </p>
...
kbl(snapshot_df,
  ...
  kableExtra::footnote(general = "", general_title = "", symbol = c(trigger_table_footnote))
...
<p style="color:black;font-size:16px;text-align:left;"> This section examines the likelihood of a framework activation in a given year under various scenarios.</p>
...
kbl(framework_df,
  ...
  kableExtra::footnote(general = "", general_title = "", symbol = c(framework_table_footnote))
...
<p style="color:black;font-size:16px;text-align:left;"> This table reports the performance metrics for the predictive triggers per timepoint. </p>
...
# read in performance metrics file
perf_metrics_per_month_data <- read.csv(perf_metrics_per_month_filename,
                              colClasses = c('character', 'character', 'character', 'numeric', 'integer')
                              )

# format performance metrics values
perf_metrics_per_month_data$value <- round(100 * perf_metrics_per_month_data$value, 0)

# compute width and position of CIs for trigger at timepoint: Jan
jan_var <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'var', trigger_id = 'Jan') # valid activation rate 
jan_det <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'det', trigger_id = 'Jan') # detection rate
jan_far <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'far', trigger_id = 'Jan') # false alarm rate
jan_mis <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'mis', trigger_id = 'Jan') # miss rate

# compute width and position of CIs for trigger at timepoint: Feb
feb_var <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'var', trigger_id = 'Feb') # valid activation rate 
feb_det <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'det', trigger_id = 'Feb') # detection rate
feb_far <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'far', trigger_id = 'Feb') # false alarm rate
feb_mis <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'mis', trigger_id = 'Feb') # miss rate

# compute width and position of CIs for trigger at timepoint: Mar
mar_var <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'var', trigger_id = 'Mar') # valid activation rate 
mar_det <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'det', trigger_id = 'Mar') # detection rate
mar_far <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'far', trigger_id = 'Mar') # false alarm rate
mar_mis <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'mis', trigger_id = 'Mar') # miss rate

# compute width and position of CIs for trigger at timepoint: Apr
apr_var <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'var', trigger_id = 'Apr') # valid activation rate 
apr_det <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'det', trigger_id = 'Apr') # detection rate
apr_far <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'far', trigger_id = 'Apr') # false alarm rate
apr_mis <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'mis', trigger_id = 'Apr') # miss rate

# compute width and position of CIs for trigger at timepoint: May
may_var <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'var', trigger_id = 'May') # valid activation rate 
may_det <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'det', trigger_id = 'May') # detection rate
may_far <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'far', trigger_id = 'May') # false alarm rate
may_mis <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'mis', trigger_id = 'May') # miss rate

# compute width and position of CIs for trigger at timepoint: June
jun_var <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'var', trigger_id = 'June') # valid activation rate 
jun_det <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'det', trigger_id = 'June') # detection rate
jun_far <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'far', trigger_id = 'June') # false alarm rate
jun_mis <- computeCIwidths(perf_metrics_df = perf_metrics_per_month_data, metric_name = 'mis', trigger_id = 'June') # miss rate
