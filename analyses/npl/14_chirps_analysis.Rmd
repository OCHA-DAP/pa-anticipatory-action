...
print(paste0("The False Positive Rate is: ", round(FPRate*100, 2), "%."))
...
print(paste0("The False Positive Rate is: ", round(FPRate2*100, 2), "%."))
...
# Comparing CHIRPS Event and GLOFAS Event
...
FloodCheckFP3 <- AggGLOFAS %>% 
    ...
    full_join(AggNational3, by = "Basin") %>%
    # rain event happens before flood event
    mutate(TimeDiff = difftime(Date.x, Date.y, units = "days"), 
           WeekDiff = difftime(Date.x, Date.y, units = "weeks"), 
           FPCheck = ifelse(TimeDiff >= 0 & TimeDiff <= 30, T, F)) %>%
    # for each reported date of flooding
    group_by(Date, Basin) %>%
    summarise(TPVals = sum(FPCheck, na.rm = T))
