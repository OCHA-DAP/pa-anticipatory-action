...
nplSF <- st_read(paste0(Sys.getenv("AA_DATA_DIR"), "/private/exploration/npl/unrco/shapefiles/Major_River_Basins.shp"))
...
read_files <- function(filePath, fileName){
    ...
    DataCube <- ncvar_get(ncObj, "precipitation") %>%
        data.frame() %>%
        bind_cols(paste0(round(ncvar_get(ncObj, "X"), 3), "E")) %>%
        setNames(c(paste0(round(ncvar_get(ncObj, "Y"), 3), "N"), "Longitude")) %>%
