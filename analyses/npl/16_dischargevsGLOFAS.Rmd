...
FloodCheckpos <- summaryFXN(NPLGFAnnualMax, NPLFlowAnnualMax, "Time", "Dates") %>%
    # Date of data to be checked
    group_by(Basin, Time) %>%
    summarise(TPVals = sum(Check, na.rm = T))
FloodCheckneg <- summaryFXN(NPLGFAnnualMax, NPLFlowAnnualMax, "Time", "Dates") %>%
    #Date of ground truth
    group_by(Basin, Dates) %>%
    summarise(TNVals = sum(Check, na.rm = T))
...
Using the annual maximums results in higher TP values. 