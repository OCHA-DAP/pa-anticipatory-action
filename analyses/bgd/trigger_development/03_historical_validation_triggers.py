...
def calculate_activations(days_above, ndays_threshold):
    ...
    for i, day in enumerate(days_above):
        ...
        start_group = True
    activations["ndays"] = (
        activations["end_date"] - activations["start_date"]
    ).dt.days + 1
...
glofas_df["dis24_Noonkhawa"].plot(
    label="GLOFAS water discharge", ax=ax1, c="green"
)
ax1.axhline(
    y=Discharge_threshold,
    c="green",
    ls="--",
    label="GLOFAS discharge threshold",
)
ax1.legend(loc="best")
...
# GLOFAS
GLOFAS_activations = calculate_activations(
    glofas_df[glofas_df["dis24_Noonkhawa"] >= Discharge_threshold].index,
    ndays_threshold_glofas,
)
for iactivation, (_, activation) in enumerate(GLOFAS_activations.iterrows()):
    ...
    ax1.axvspan(
        activation["start_date"] - timedelta(days=0.5),
        activation["end_date"] + timedelta(days=0.5),
        facecolor="green",
        alpha=0.5,
    )
# FFWC
FFWC_activations = calculate_activations(
    ffwc_his_df[ffwc_his_df["WL"] >= Water_threshold].index,
    ndays_threshold_ffwc,
)
