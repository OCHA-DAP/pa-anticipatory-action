...
# This is a stand-alone script to perform basic processing to generate shapefiles
# to represent surface water extent for a given area of interest.

# TO BEGIN: Download the global surface water (GSW) tiles (Seasonality 2019) that you want from
# https://global-surface-water.appspot.com/download
# and place them within a single directory.

# This script will then:
# 1) Resample the GSW raster files to downscale for faster processing
# 2) Select areas with water coverage depending on the desired months of coverage
# 3) Filter out water areas that are smaller than a given threshold (in sq meters)
# 4) If desired, clip the water areas according to a shapefile, with potential for a buffer to be specified
# or to clip to the bounding box around the shapefile rather than the specific geometry
# 5) Export the remaining water areas to a shapefile (projected to the specified UTM zone)

# SET PARAMETERS: Set the following parameters prior to running the script
RFAC = 0.25  # Resample factor (inverse, so 0.25 for example, will increase cell size X4)
...
DATA_DIR = 'analyses/bangladesh/data/GSW_data/'  # Directory where the GWS .tif files are stored
UTM = 'EPSG:32646'  # UTM zone for the area of interest
CLIP = True  # Whether or not to clip the output by a given shp
CLIP_FILE = 'analyses/bangladesh/data/ADM_Shp/bgd_admbnda_adm0_bbs_20201113.shp'  # Path location to the clipping shapefile
...
def process_tif(fname, factor):
    resampled, transform = resample_raster(os.path.join(DATA_DIR, fname), factor)
    print(f'Processing {fname}, resampled to {resampled.shape} cells')
    data_reclass = np.where(((resampled >= MIN_MONTHS) & (resampled <= 12)), 1, 0)
    mask = None
    results = (
...
def clip_vector(to_clip, clip_geom, buffer_dist=0, bound_box=True):
    ...
    if bound_box:  # Clip to the bounding box, rather than specific geometry
        clip_geom = clip_geom.envelope
    return gpd.clip(to_clip, clip_geom.envelope)


if __name__ == '__main__':
    files = [process_tif(f, RFAC) for f in os.listdir(DATA_DIR) if f.endswith('.tif')]
