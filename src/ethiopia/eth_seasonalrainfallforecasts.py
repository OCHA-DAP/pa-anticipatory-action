...
def main(download, config=None):
    ...
    if forecmonth>pubmonth:
        ...
        forecyear=pubyear+1
    leadtime_cf_nmme=cftime.Datetime360Day(forecyear, forecmonth, 1, 0, 0, 0, 0)
    ...
    output_dir=os.path.join(config.DATA_DIR,config.PUBLIC_DIR,config.PROCESSED_DIR,config.iso3_code, config.PLOT_DIR)
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    adm_path=os.path.join(config.DATA_DIR,config.PUBLIC_DIR,config.RAW_DIR,config.iso3_code,config.SHAPEFILE_DIR, parameters['path_admin1_shp'])
    ...
    iri_ds_clip = iri_ds_sel.rio.set_spatial_dims(x_dim="lon",y_dim="lat").rio.clip(df_bound.geometry.apply(mapping), df_bound.crs,
                                                                all_touched=True)
    ...
    icpac_ds_clip = icpac_ds.rio.set_spatial_dims(x_dim="lon",y_dim="lat").rio.clip(df_bound.geometry.apply(mapping), df_bound.crs,
                                                                all_touched=True)
    ...
    nmme_ds_clip = nmme_ds_sel.rio.set_spatial_dims(x_dim="lon",y_dim="lat").rio.clip(df_bound.geometry.apply(mapping), df_bound.crs,
                                                                all_touched=True)
    ...
    fig_clip=plot_raster_boundaries_clip([iri_ds_sel,icpac_ds,nmme_ds_sel],adm_path,cmap="YlOrRd",predef_bins=bins,title_list=["IRI","ICPAC","NMME"],suptitle=f"Probability of below average rainfall \n for forecasts published in {pubmonth_abbr} {pubyear}, forecasting {forecseason} ({leadtime} month leadtime)",legend_label="Probability below average precipitation")
    # fig_clip.savefig(os.path.join(output_dir, f'IRIICPACNMME_Cbelow_clipped.png'),
    #                  format='png', bbox_inches='tight')

    bins=[0,40,45,50,55,60,70,100]
    fig_clip=plot_raster_boundaries_clip([icpac_ds],adm_path,cmap="YlOrRd",predef_bins=bins,suptitle=f"ICPAC rainfall probabilistic forecast for March-May 2021, published in {pubmonth_abbr} {pubyear}",legend_label="Probability of below-average rainfall")
