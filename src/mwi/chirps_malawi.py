...
# get_chirps_data_daily,
from src.indicators.drought.config import Config
from src.indicators.drought.utils import parse_args
from src.utils_general.utils import config_logger


# Note: this script is a mess. But since it is being refactored for
# aa-toolbox soon, we are leaving it as it is
def main(download, config=None):
    ...
    for year in years:
        download_chirps_daily(
            config=config, year=year, resolution="05", clobber=clobber
        )
    ...
    # adm1_path = os.path.join(
    #     country_data_raw_dir,
    #     config.SHAPEFILE_DIR,
    #     parameters["path_admin1_shp"],
    # )
    # for i in years:
    #     # if only want to download uncomment everythin except the next
    #     # line download_chirps(config, i, resolution)
    #     ds, transform = get_chirps_data_daily(
    #         config, i, download=download, resolution=resolution
    #     )
    #     df_bound = gpd.read_file(adm1_path)
    #     # clip global to malawi to speed up calculating rolling sum
    #     ds_clip = ds.rio.set_spatial_dims(
    ...
    #         np.sort(
    #             np.unique(
    #                 ds_roll.precip.values.flatten()[
    #                     ~np.isnan(ds_roll.precip.values.flatten())
    #                 ]
    #             )
    #         )
    #     )
    # #select november month and plot those. Just because it speeds
    # up plotting compared to 365 days ds_roll_sel =
    # ds_roll.sel(time=slice(f"{i}-11-01", f"{i}-11-30")) ds_sel =
    # ds_clip.sel(time=slice(f"{i}-11-01", f"{i}-11-30"))
    # predef_bins=np.linspace(ds_sel.precip.min(),ds_roll_sel.precip.max())

    # get the raster cell with the maximum rainfall for each date
    # (can also do analysis when doing min instead)
    ...
    # # #function is not optimized yet, xlabels unreadable but gives
    # # an idea
...
"""
    #want 2D array for plot_raster_boundaries. Without squeeze it
    remains 3D ds_3011=ds.sel(time="2019-11-30").squeeze()

    fig_bound = plot_raster_boundaries(ds_3011, country, parameters,
    config,forec_val="precip")
    fig_bound.savefig(os.path.join(output_dir,"chirps_20191130_boundaries.png"),
    format='png',bbox_inches='tight')

    ds_sel=ds.sel(time=slice("2019-11-01","2019-11-30")) print(ds_sel)
    ...
    fig_histo.savefig(os.path.join(output_dir,"chirps_201911_histogram.png"),
    format='png',bbox_inches='tight')
    fig_dates.savefig(os.path.join(output_dir,"chirps_201911_rastervalues.png"),
    format='png',bbox_inches='tight')
"""
