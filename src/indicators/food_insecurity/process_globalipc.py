...
def test_mismatch_adminlevels(
    ...
    The method we use is to compute the totals by aggregation of adm2
    but raise a warning if doesn't match with reported adm0/1 numbers to
    make user aware But the numbers on admin0 and admin1 level are also
    ...
    if admin_level == 0:
        ...
        # rows corresponds to those that don't contain the admin0 name
        df_precalc = df_precalc[
            ~df_precalc[config.ADMIN0_COL]
            .str.lower()
    ...
    if not df_precalc.empty and not df_agg.empty:
        df_merge = df_agg.merge(
            df_precalc,
            on=["date", config.ADMIN0_COL],
            suffixes=("_agg", "_precalc"),
        )
        # only look at population and perc_ipc3+ for now, could add more
        match_cols = [f"pop_{p}" for p in config.IPC_PERIOD_NAMES] + [
            f"perc_{p}_3p" for p in config.IPC_PERIOD_NAMES
        ]
        for c in match_cols:
            # select rows where the two methods differ by more than 5%
            if "perc" in c:
                df_notmatch = df_merge[
                    abs(df_merge[f"{c}_agg"] - df_merge[f"{c}_precalc"]) > 5
                ]
            else:
                df_notmatch = df_merge[
                    abs(
                        (df_merge[f"{c}_agg"] - df_merge[f"{c}_precalc"])
                        / df_merge[f"{c}_precalc"]
                    )
                    > 0.05
                ]
            if not df_notmatch.empty:
                # raise warning if any rows that differ by more than 5%
                dateadm_notmatch = df_notmatch.set_index(
                    ["date", config.ADMIN0_COL]
                ).index.unique()
                dateadm_notmatch = dateadm_notmatch.sort_values()
                dateadm_notmatch_str = ",".join(
                    [
                        f"[{da[0].strftime('%m-%Y')},{da[1]}]"
                        for da in dateadm_notmatch
                    ]
                )
                logger.warning(
...
def aggregate_adminlevel(df_ipc, admin_level, country, config):
    ...
    Returns: df_ipc_agg (pd.DataFrame): dataframe with data aggregated
        to admin_level
    """
    # we assume that the admin2 column is nan for rows where adm1/0 sums
    # are reported. Thus, this enable us to compute the total adm1/0s by
    # summing the adm2s From empirical testing it was shown that the
    # aggregations from adm2 to adm1/0 don't always equal the reported
    # adm1/0 sums Since one methodology had to be chosen, we chose to
    # always use the adm2 numbers as basis and aggregate from here This
    # enables us to be consistent across admin levels The reasons for
    # disagreement are not entirely clear. One cause is the inclusion of
    # IDPs on adm1 and not on adm2, but this doesn't explain all the
    # differences
    df_ipc_adm2 = df_ipc[
        (df_ipc["date"].notnull()) & (df_ipc["ADMIN2"].notnull())
    ]
    if admin_level == 0:
        ...
        # region for each admin2, but that the summed numbers of adm1s
        # are given in the adm0 column and thus not in the admin1 thus
        # we can groupby the admin1 col to get the sums of the adm2s per
        # adm1
        df_ipc_agg = df_ipc_adm2.groupby(
        ...
        # groupby adm1-adm2 combination and not only adm2
        df_ipc_agg = df_ipc_adm2.groupby(
            ["date", config.ADMIN1_COL, config.ADMIN2_COL],
            dropna=False,
            as_index=False,
        ).sum()
        df_ipc_agg[config.ADMIN0_COL] = country
...
def compute_population_admin(df, admin_level, config):
    """Compute the population assigned to an IPC phase per admin-date
    combination Global IPC reports the population that is analysed but
    this reported number doesn't always equal the sum of population over
    all the ipc phases Why there is this discrepancy, isn't entirely
    clear.

    But since the threshold are based on percentage of the population in
    each phase, we want to make sure the total analysed population
    equals the sum of the population of all ipc phases. Args: df
    (pd.DataFrame): dataframe containing the populatoin per ipc phase
    and the reported total population admin_level (int): integer
    indicating which admin level to aggregate to config (Config): Config
    class Returns: df (pd.DataFrame): input dataframe with column per
    period containing the sum of population of all IPC phases
    """

    for period in config.IPC_PERIOD_NAMES:
        # with min_count=1 NaN is returned instead of 0 if all values are NaN
        df[f"pop_{period}"] = df[[f"{period}_{i}" for i in range(1, 6)]].sum(
            axis=1, min_count=1
        )
        # raise a warning if the reported numbers and the sum over the
        # ipc phases are not equal. only do this for admin2 since we
        # aggregate from those user doesn't have to do anything with
        # this information, but good to know as a quality indicator of
        # the data
        if admin_level == 2:
            df_notmatch = df[
                abs(
                    (df[f"pop_{period}"] - df[f"reported_pop_{period}"])
                    / df[f"reported_pop_{period}"]
                )
                > 0.05
            ]
            df_notmatch = df_notmatch[
                (df_notmatch[config.ADMIN2_COL].notnull())
            ]
            if not df_notmatch.empty:
                dateadm_notmatch = df_notmatch.set_index(
                    ["date", f"ADMIN{admin_level}"]
                ).index.unique()
                dateadm_notmatch = dateadm_notmatch.sort_values()
                dateadm_notmatch_str = ",".join(
                    [
...
def compute_ml_period(df_ipc_agg, df_ipc):
    """
    Retrieve the ML1 and ML2 period for each date. This is included in
    the data, but for some dates the whole country might not have
    ml1/ml2 data and for some dates some admins might not have ml1/ml2
    data Therefore, we cannot do a simple groupby of ML1 and ML2 period
    when aggregating So check all unique values and only assign this
    value if there is one unique value that is not nan. Args:
    df_ipc_agg: aggregated dataframe on admin_level df_ipc: original
    dataframe

    Returns: df_ipc_agg: aggregated dataframe on admin_level with ML1
        and ML2 period column
    """
...
def download_globalipc(country, config, parameters, output_dir):
    ...
    # first year to retrieve data for. Doesn't matter if global ipc only
    # started including data for later years
    min_year = 2010
    # last date to retrieve data for. Doesn't matter if this is in the
    # future
    ...
    try:
        ...
        logger.warning(
            f"Cannot download GlobalIPC data for {parameters['iso3_code']}"
        )
...
def process_globalipc(
    ...
    # remove rows with nan date, these often include textual information
    # in other formats
    df_ipc = df_ipc[df_ipc["date"].notnull()]
    # df_ipc_adm2 =
    # compute_population_admin(df_ipc_adm2,admin_level,config)

    # we map names but don't assign admin1's to admin2's if they are
    # missing from the file (e.g. MWI)
    if len(df_ipc[f"ADMIN{admin_level}"].dropna().unique()) == 0:
        ...
        # recompute to be sure they match
        ...
        if "globalipc_adm_mapping" in parameters["foodinsecurity"].keys():
            globalipc_bound_mapping = parameters["foodinsecurity"][
                "globalipc_adm_mapping"
            ]
            df_ipc_agg = df_ipc_agg.replace(globalipc_bound_mapping)
        # give warning on not matching adm names global ipc and
        # boundaries file assume admin0 name is correct, cause quickly
        # causes disrepancies due to added strings in Global IPC adm0
        # name
        if admin_level != 0:
            compare_adm_names(
                df_ipc_agg,
        ...
        # aggregation from adm2 to adm0/1 doesn't always equal the
        # numbers reported directly on adm0/1 raise a warning if this
        ...
        #  analysis Would have to use WorldPop data for that, see
        #  process_fewsnet_worldpop.py
...
def retrieve_globalipc(
    ...
    globalipc_dir = os.path.join(
        country_data_raw_dir, config.GLOBALIPC_RAW_DIR
    )
