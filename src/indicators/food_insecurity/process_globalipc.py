...
def test_mismatch_adminlevels(
    ...
    """There might be a mismatch between different levels of reporting on
    admin0 and admin1 level.
...
def aggregate_adminlevel(df_ipc, admin_level, country, config):
    ...
    df_ipc_adm2 = df_ipc[
        (df_ipc["date"].notnull()) & (df_ipc[f"ADMIN2"].notnull())
    ]
    if admin_level == 0:
...
def compute_population_admin(df, admin_level, config):
    ...
    combination Global IPC reports the population that is analysed but this
    reported number doesn't always equal the sum of population over all the ipc
    phases Why there is this discrepancy, isn't entirely clear.

    But since the threshold are based on percentage of the population in each phase,
...
def download_globalipc(country, config, parameters, output_dir):
    ...
    max_year = (
        datetime.now().year
    )  # last date to retrieve data for. Doesn't matter if this is in the future
