...
class Config:
    ...
    def __init__(self):
        ...
        self.FOODINSECURITYDATA_DIR = os.path.join(
            self.DATA_DIR, "raw", "food_insecurity"
        )
    ...
    FEWSNET_DATES = (
        ...
        + [
            f"{str(i)}{m}"
            for i in range(2016, int(TODAY_YEAR) + 1)
            for m in ["02", "06", "10"]
        ]
    )
    # Worldpop
