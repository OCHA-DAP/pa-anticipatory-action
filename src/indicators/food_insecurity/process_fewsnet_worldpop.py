...
def compute_total_admin_population(df_fews, admin_path, pop_path, config):
    # calculate total population of every admin, to use for comparison
    # of population given by intersection of admin shape and fewsnet
    ...
    for period in config.IPC_PERIOD_NAMES:
        ...
        # calculate total population per period, also those that don't
        # have an ipc phase assigned these can include 66 = water, 88 =
        # parks, forests, reserves, 99 = No Data or Missing Data
        period_cols_all = [
        ...
        # there can be a slight disperancy between the fewsnet and admin
        # shapefile. Since we take the intersection of the fewsnet and
        # admin shapefile, some small areas might hence be lost here we
        # calculate the total population based on the admin shape, and
        # compare it to the total population of the overlay of the admin
        # and fewsnet shapefiles if the discrepancy causes more than 5%
        # of the population to be excluded, raise a warning
        pop_admfews = (
        ...
        # In some cases a large fraction of the country's population
        # doesn't have an IPC phase assigned (mostly 99). This mostly
        # occurs if FewsNet didn't cover the country. Due to small
        # disreperancies between the FN and admin shapefile a small
        # fraction can then still be assigned to an IPC phase. If more
        # than 50% doesn't have a phase assigned, raise a warning TODO:
        # if perc_ipcclass is really small for the country fewsnet data,
        # check if region fewsnet data does contain data. Not sure how
        # to.. But for SOM 201307 ML2 it is the case that the country
        # fewsnet data does contain data but the east-africa not. Might
        # happen other way around as well.. TODO: return NaN instead of
        # zeroes if no data
        perc_ipcclass = (
...
def assign_population_fewsnet(
    ...
    # assign fewsnet area (livelihood) per admin region in df_adm.
    # Sometimes livelihoods cross admin boundaries, in that case overlay
    # splits the livelihood at the admin boundaries TODO: overlay takes
    # really long to compute, but could not find a better method only
    # use geometry and period since in newer FN data there might also
    # admin columns be present that interfere with the admin columns of
    # df_adm
    df_fewsadm = gpd.overlay(
        df_adm, df_fews[["geometry", period]], how="intersection"
    )
    # encountered with Chad (2018-06) that all polygons were included
    # twice in the data. Not sure if this fixes all future cases, but
    # only keep first occurence of same polygon and IPc phase seems safe
    # enough to do
    df_fewsadm = df_fewsadm.drop_duplicates(subset=["geometry", period])
    # calculate population per "geometry", where geometry is the area of
    # a livelihood within an admin zone In pop_path, the value per
    # raster cell is the population of that cell, so sum the cells
    # within a geometry to get the total population in the calculation a
    # cell is considered to belong to an area if the center of that cell
    # is inside the area. see
    # https://pythonhosted.org/rasterstats/manual.html#rasterization-strategy
    # for more explanation on the computation of zonal_stats
    ...
    # not all IPC levels will be present in all admin regions, so fill
    # those with zeroes
...
def process_fewsnet_worldpop(
    ...
    """Retrieve all FewsNet data, and calculate the population per IPC
    ...
    for d in pbar:
        ...
        for period in config.IPC_PERIOD_NAMES:
            ...
            # path to fewsnet data sometimes fewsnet publishes per
            # region, sometimes per country if the country file exists,
            # we prefer that one since it sometimes contains more
            # detailed livelihood zones
            fews_path = None
            fews_region_filename = config.FEWSNET_FILENAME.format(
                region=region.lower(),
                regionabb=regionabb.upper(),
                date=d,
                period=period.upper(),
            )
        ...
        if df_fews_list:
            # concat the dfs of the different periods, with an unique
            # entry per date-adm1-adm2 combination
            df_listind = [
                df.set_index(adm_cols + ["date"]) for df in df_fews_list
            ]
            df_comb = pd.concat(df_listind, axis=1).reset_index()

            # add ipc phase columns that are not present in the data
            # (commonly phase 5 columns)
            ipc_cols = [
                f"{period}_{i}"
                for period in config.IPC_PERIOD_NAMES
                for i in range(1, 6)
...
def retrieve_fewsnet_worldpop(
    ...
    region combination. The IPC phase is retrieved from the FewsNet
