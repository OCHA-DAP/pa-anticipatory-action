...
def compute_raster_stats(
    ...
    gdf,
    gdf_id_col,
    ds_thresh_list=[2, 4, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50],
):
    """
    ...
    for d in rainy_dates:
        ...
        if use_cache and output_path.exists():
            ...
            if not chirpsgefs_raster_filepath.exists():
                ...
                df = compute_raster_stats(
                    rds.sel(band=1), rds.rio.transform(), gdf_adm, adm_col
                )
                df.loc[:, "date"] = d
...
def get_rainy_season_dates(
    rainy_season_path, onset_col="onset_date", cessation_col="cessation_date"
):
    """
