...
class TestDownload(unittest.TestCase):
    def setUp(self):
        ...
        self.expected_hours = [f"{str(x).zfill(2)}:00" for x in range(24)]
    ...
    def test_ecmwf_era5_download(self, fake_mkdir, fake_retrieve):
        ecmwf_era5 = ecmwf.EcmwfEra5()
        ecmwf_era5.download(
            country_iso3=self.country_iso3,
            area=self.area,
            year_min=self.year,
        ...
        expected_args = {
            ...
            "request": {
                ...
                "day": self.expected_days,
                "area": self.expected_area,
                "time": self.expected_hours
            },
            "target": Path(
                f"/tmp/public/raw/{self.country_iso3}/ecmwf/reanalysis-era5-single-levels"
                f"/{self.country_iso3}_reanalysis-era5-single-levels_2000.grib"
            ),
        }
        fake_retrieve.assert_called_with(**expected_args)
